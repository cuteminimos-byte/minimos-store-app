...
export default function TrendingSection() {
  ...
  const trendingProducts: Product[] = [
    { id: 1, title: "Car Shorts", price: "Rs.3,250.00", numericPrice: 3250, sku: "GKCS-1", src: "/images/image11.jpeg", link: "/trending/car-shorts", isNew: true },
    { id: 2, title: "Ruby Rani -4pc", price: "Rs.8,050.00", numericPrice: 8050, sku: "GKMR-1", src: "/images/image13.jpeg", link: "/trending/ruby-rani" },
    { id: 3, title: "Shehnai -4pc G", price: "Rs.10,550.00", numericPrice: 10550, sku: "GKSN-1", src: "/images/image9.jpeg", link: "/trending/shehnai-4pc" },
    { id: 4, title: "Traditional Festive Wear", price: "Rs.9,200.00", numericPrice: 9200, sku: "GKTF-1", src: "/images/image16.jpeg", link: "/trending/festive-wear" },
  ];
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white rounded-xs overflow-hidden max-w-[760px] w-full relative shadow-2xl flex flex-col md:flex-row animate-in fade-in zoom-in-95 duration-200">
            <button onClick={closeQuickView} className="absolute top-0 right-0 z-50 bg-black text-white w-10 h-10 flex items-center justify-center hover:bg-neutral-800 cursor-pointer">
            ...
            <div className="w-full md:w-1/2 p-5 flex flex-col justify-between bg-white border-r border-gray-100">
              <div className="relative aspect-square w-full rounded-xs overflow-hidden group/modalimg cursor-pointer">
                <Image src={selectedProduct.src} alt={selectedProduct.title} fill className="object-cover object-center transition-transform duration-500 group-hover/modalimg:scale-106" unoptimized />
              </div>
            ...
            <div className="w-full md:w-1/2 p-6 flex flex-col justify-start overflow-y-auto max-h-[80vh] md:max-h-none">
              <h2 className="text-xl font-serif font-bold text-gray-900 leading-tight">{selectedProduct.title}</h2>
              <p className="text-[11px] text-gray-400 font-sans mt-1 uppercase">SKU: {selectedProduct.sku}</p>
              <p className="text-lg font-bold text-gray-900 font-sans mt-3">{selectedProduct.price}</p>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2 max-w-[340px]">
                {sizesList.map((size) => (
                  <button key={size} onClick={() => setSelectedSize(size)}
                    className={`px-2.5 py-1.5 text-[11px] font-medium rounded-full border transition-all cursor-pointer min-w-[52px] text-center ${selectedSize === size ? 'border-black bg-white text-gray-900 font-bold ring-1 ring-black' : 'border-gray-300 text-gray-700 hover:border-gray-900 bg-white'}`}>
                    {size}
                  </button>
                ))}
              </div>
              ...
              <div className="mt-4">
                ...
                <div className="inline-flex items-center border border-gray-400 rounded-xs bg-white">
                  <button onClick={() => setQuantity(prev => Math.max(1, prev - 1))} className="px-3 py-1 text-gray-600 hover:bg-gray-50 text-base font-medium cursor-pointer">&minus;</button>
                  <span className="w-10 text-center font-sans text-xs font-bold text-gray-900">{quantity}</span>
                  <button onClick={() => setQuantity(prev => prev + 1)} className="px-3 py-1 text-gray-600 hover:bg-gray-50 text-base font-medium cursor-pointer">&#43;</button>
                </div>
              </div>
              ...
              <div className="mt-4 border-t border-gray-100 pt-3">
                <div className="text-xs text-gray-700 flex gap-1">
                  <span>Subtotal:</span>
                  <span className="font-bold text-gray-900">Rs.{(selectedProduct.numericPrice * quantity).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                </div>
              </div>

              <div className="mt-5 space-y-2.5">
                <button className="w-full bg-[#1c1c1c] text-white border border-[#1c1c1c] font-sans text-xs font-bold uppercase tracking-wider py-3.5 rounded-xs hover:bg-white hover:text-black transition-colors duration-300 cursor-pointer">Add to Cart</button>
                <button onClick={handleBuyItNow} className="w-full bg-white text-gray-900 border border-gray-900 font-sans text-xs font-bold uppercase tracking-wider py-3.5 rounded-xs hover:bg-black hover:text-white transition-colors duration-300 cursor-pointer">Buy It Now</button>
              </div>
            </div>
          </div>
        </div>
